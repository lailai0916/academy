...
目标范围是浙江高考语文、数学、英语、物理、化学、技术六科。目前可用的是英语词汇与古诗词记忆训练、间隔复习和 AI 讲解，以及首个可恢复的动量定理连续课程；其他学科课程仍在规划。

[项目简介](docs/project-brief.md) · [项目说明](docs/project-overview.md) · [PDF](output/pdf/academy-project-brief.pdf) · [全部文档](docs/README.md)

## 项目特性

🧭 **六科课程架构** — 语文、数学、英语、物理、化学、技术使用统一课程目录。课程教学、独立测评和记忆训练保持各自的数据模型，再由长期计划组合任务。

⚛️ **连续课程试用** — 动量定理课程包含基础检查、分步课件、练习提示、课中 AI 追问、无提示测评和七天延迟复测。课程进度与每次作答保存在服务端，退出后可以继续。
...
🖥️ **公开官网与学习空间** — 公开页和登录后的课程页会区分“已实现”“试用中”和“规划中”。学习空间带分区侧栏、顶部搜索、通知和主题控制，支持浅色、深色与跟随系统主题。
...
接下来先用已经接入的[动量定理课程](docs/learning-pilot.md)完成真实学习，核对内容、判分、提示依赖与模型费用，再把有效结构扩展到后续专题。

之后随自己的学习进度增加课程，逐步安排语文、数学、英语、物理、化学、技术六科。具体工作见[后续计划](docs/project-roadmap.md)。

## 技术架构

npm workspace 将 React 客户端、Fastify API 与共用验证模型分离。现有 FSRS 卡片只服务词汇与古诗词记忆训练，连续课程使用独立的课程运行与学习证据模型。Caddy 提供网页并把
