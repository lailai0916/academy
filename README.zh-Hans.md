...
`lailai's Academy` 是一个个人学习工作台，通过每日看板、复习模块和本地 AI 规划预览，
把长期目标拆成每天可执行的练习。
...
🧠 **规划预览** —— 将学习目标转换成简短、明确标注的本地计划。

📖 **学习模块** —— 在 Words、Poems 和 Review 之间切换，不离开当前工作台。

💾 **本地持久化** —— 通过浏览器的 `localStorage` 保存任务进度。
...
规划器当前生成的是本地预览。未来可以通过 `VITE_API_BASE_URL` 接入 API；密钥必须留在
服务端边界内。
